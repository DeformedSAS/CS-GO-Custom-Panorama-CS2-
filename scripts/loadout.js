...
var Loadout = ( function() {
    ...
    var _AddSort = function()
    {
        ...
        for ( var i = 0; i < count; i++ ) 
        {
            ...
            var newEntry = $.CreatePanelWithProperties( 'Label', elDropdown, sort, {
                class: 'DropDownMenu'
            } );
    ...
    var _OnShuffleToggle = function()
    {
        ...
        $.GetContextPanel().FindChildInLayoutFile( 'LoadoutShuffleOptions' ).visible = elBtn.checked;
    };
    ...
    function _GetModelPanel ()
    {
        var loadoutSlot = _GetSelectededLoadoutSlot();
        var elItemContainer = $.GetContextPanel().FindChildInLayoutFile( loadoutSlot );

        return InspectModelImage.GetModelPanel( );
    }

    function _InitPreviewPanel ( loadoutSlot )
    {
        var team = _GetSelectedTeamAsString( loadoutSlot );
        var itemId = LoadoutAPI.GetItemID( team, loadoutSlot );

        var elItemContainer = $.GetContextPanel().FindChildInLayoutFile( loadoutSlot );
        InspectModelImage.Init( elItemContainer, itemId );
    }
    ...
    var _UpdateSingleItem = function()
    {
        ...
        arrPanels.forEach( panelName => 
        {
            $.GetContextPanel().FindChildInLayoutFile( panelName ).SetHasClass( 'hidden', panelName != loadoutSlot );
    ...
    var _FillOutItemData = function( itemId, loadoutSlot )
    {
        ...
        _InitPreviewPanel( loadoutSlot );
        _InitCharacterItemPanel( itemId );

        var elModel = _GetModelPanel();
        ...
        elModel.SetHasClass( 'hidden', false );
        elRarity.SetHasClass( 'hidden', false );
    ...
    var _InitCharacterItemPanel = function( charItemId )
    {
        ...
        var elModelPanel = InspectModelImage.GetModelPanel( );

        var elCharacterButtons = $( '#id-character-buttons-container' ).FindChildInLayoutFile( 'id-character-buttons' );
        CharacterButtons.InitCharacterButtons( elCharacterButtons, elModelPanel, characterToolbarButtonSettings );
    }
    ...
    var _OnReadyForDisplay = function()
    {
        ...
        var loadoutSlot = _GetSelectededLoadoutSlot();
        $.DispatchEvent( "ShowLoadout", loadoutSlot, Number( _GetSelectededTeam( loadoutSlot ) ) );
        Loadout.PlayerEquipSlotChangedHandler = $.RegisterForUnhandledEvent( 'PanoramaComponent_Inventory_PlayerEquipSlotChanged', Loadout.OnEquipUpdate );
    ...
    var _OnEquipUpdate = function( pos, subslot, oldItemId, newItemId)
    {
                                 
        if ( oldItemId == newItemId )
            return;

        _UpdateInteriorPreviewAndItemTiles(oldItemId, newItemId);
    }
    
    var _UpdateInteriorPreviewAndItemTiles = function (oldItemId, newItemId)
    {
        if ( _GetUsesWedges() )
        {
            var loadoutSlot = _GetSelectededLoadoutSlot();
    ...
    return {
        ...
        InitPreviewPanel                    : _InitPreviewPanel
    };
