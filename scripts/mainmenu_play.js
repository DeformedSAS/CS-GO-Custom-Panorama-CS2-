    ...
    function UpdateIconsAndScreenshots(p, numTiles, mapGroupName, mg) {
        ...
        for (let i = 0; i < keysList.length; i++) {
            ...
            if (m_gameModeSetting === 'survival') {
                ...
                mapImage.style.backgroundImage = 'url("file://{images}/map_icons/screenshots/1080p/' + keysList[i] + '.png")';
            }
            mapImage.style.backgroundPosition = '50% 0%';
...
function _ShowCancelSearchButton(isSearching, isHost) {
    ...
    if (!btnCancel.enabled) {
        // Perform an action if the button is not enabled
        btnCancel.visible = false; // Example: Hide the button if not enabled
    } else {
        btnCancel.visible = true;  // Ensure the button is visible if enabled
    }
}
...
function _LazyCreateWorkshopTab() {
    ...
    // Simulate WorkshopAPI.GetAvailableWorkshopMaps with mock data
    const arrMaps = [
        {
            workshop_id: '123456789',
            map: 'de_dust2',
            name: 'Dust II',
            imageUrl: 'file://{images}/map_icons/screenshots/360p/de_dust2.png'
        },
        {
            workshop_id: '987654321',
            map: 'de_inferno',
            name: 'Inferno',
            imageUrl: 'file://{images}/map_icons/screenshots/360p/de_inferno.png'
        }
    ];
    ...
    function OnPressOfficialServers() {
        ...
        _ApplySessionSettings();
		// d3gk: if u select official servers, bot panel will hide
		var bot_panel = $('#BotDifficultyDropdown');
		// d3gk: check if panel is not null
		if (!bot_panel) {
			return;
		}
    ...
    function OnPressListenServers() {
        ...
        _TurnOffDirectChallenge();
        _ApplySessionSettings();
		// d3gk: if u select offline with bots stuff, bot panel will show up
		var bot_panel = $('#BotDifficultyDropdown');
		// d3gk: check if panel is not null
		if (!bot_panel) {
			return;
...
function BotDifficultyChanged() {
    ...
    // Update visibility of bot difficulty panel based on condition
    var bot_panel = $('#BotDifficultyDropdown');
    if (botDiff === 'competitive') {
        bot_panel.visible = false;
    } else {
        bot_panel.visible = true;
    }
